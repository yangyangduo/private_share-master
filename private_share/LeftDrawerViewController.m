...
- (void)viewDidLoad
{
    ...
    items = @[ @"portal", @"task", @"activity", @"mall", @"mi_repo2", @"exchange_locations", @"yoomid" ];
    ...
    CGFloat logoImageViewY = [UIDevice systemVersionIsMoreThanOrEqual7] ? 30 : 10;
    ...
    CGFloat itemsCollectionViewY = logoImageView.frame.origin.y + logoImageView.bounds.size.height + 25;
...
- (void)setSelectedItem:(NSString *)selectedItem {
    ...
    } else if([@"yoomid" isEqualToString:selectedItem]) {
        rootViewController = [[MyYoomidViewController alloc] init];
    }
