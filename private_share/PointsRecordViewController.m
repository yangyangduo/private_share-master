...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ((UILabel *)[cell viewWithTag:999]).text = [NSString stringWithFormat:@"%@ %d%@", (order.points > 0 ? @"+" : @""), order.points, NSLocalizedString(@"points", @"")];
    ...
    if(PointsOrderTypeAdTask == order.orderType) {
        orderTypeName = [NSString stringWithFormat:@"%@%@", order.introduce2, orderTypeName];
    }
    if(![XXStringUtils isBlank:order.introduce]) {
        cell.textLabel.text = [NSString stringWithFormat:@"%@(%@)", orderTypeName, order.introduce];
    } else {
